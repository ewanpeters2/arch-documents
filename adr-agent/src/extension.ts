...
const AI_PROMPTS: Record<string, TopicSuggestions> = {
  microservices: {
    positive: [
    ...
    if (request.command === 'new') {
      ...
      if (topic && AI_PROMPTS[topic]) {
        ...
        const selected = await vscode.window.showQuickPick(
          suggested.map(s => ({ label: s, picked: true })),
          { 
      ...
      if (customAlt) { alternatives.push(customAlt); }
...
function detectTopic(title: string): string | null {
  ...
  if (lower.includes('api gateway') || lower.includes('gateway')) { return 'api_gateway'; }
  if (lower.includes('lambda') || lower.includes('serverless')) { return 'lambda'; }
  return null;
}
