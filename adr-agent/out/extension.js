...
function getInterviewState(workspaceFolder) {
    return interviewStates.get(workspaceFolder);
}
function setInterviewState(workspaceFolder, state) {
    interviewStates.set(workspaceFolder, state);
}
function clearInterviewState(workspaceFolder) {
    interviewStates.delete(workspaceFolder);
}
function createEmptyInterviewState() {
    return {
        step: 1,
        title: '',
        problemStatement: '',
...
// =============================================================================
async function handleInterviewResponse(state, response, workspaceFolder, adrDir, stream) {
    switch (state.step) {
...
${optionSuggestions.options.map((o) => `- ${o}`).join('\n')}
...
${proConSuggestions.positive.slice(0, 3).map((p) => `- ✅ ${p}`).join('\n')}

**Typical cons:**
${proConSuggestions.negative.slice(0, 3).map((n) => `- ❌ ${n}`).join('\n')}
...
// =============================================================================
function extractProsAndCons(text) {
    const positive = [];
    const negative = [];
    const lines = text.split('\n');
...
async function generateAndSaveADR(input, adrDir, stream) {
    ...
    const content = ADR_TEMPLATE
        ...
        ? input.positive.map((p) => `- ✅ Good, because ${p.toLowerCase()}`).join('\n')
        : '- To be defined')
        .replace('{{NEGATIVE}}', input.negative.length > 0
        ? input.negative.map((n) => `- ❌ Bad, because ${n.toLowerCase()}`).join('\n')
        : '- To be defined')
...
{{REFERENCES}}
`;
const taInterviewStates = new Map();
function getTAInterviewState(workspaceFolder) {
    return taInterviewStates.get(workspaceFolder);
}
function setTAInterviewState(workspaceFolder, state) {
    taInterviewStates.set(workspaceFolder, state);
}
function clearTAInterviewState(workspaceFolder) {
    taInterviewStates.delete(workspaceFolder);
}
function createEmptyTAInterviewState() {
    return {
        step: 1,
        title: '',
        problemDescription: '',
...
// TA HELPER FUNCTIONS
// =============================================================================
function getNextTANumber(taDir) {
    if (!fs.existsSync(taDir)) {
        return '001';
    }
    const files = fs.readdirSync(taDir);
    const numbers = files
        .filter((f) => f.match(/^ta-\d{3}/))
        .map((f) => parseInt(f.match(/^ta-(\d{3})/)?.[1] || '0', 10));
    const max = Math.max(0, ...numbers);
    return String(max + 1).padStart(3, '0');
}
// =============================================================================
...
`);
        ...
        if (request.command === 'list') {
            ...
            const files = fs.readdirSync(taDir)
                .filter((f) => f.startsWith('ta-') && f.endsWith('.md'))
                .sort();
            const taList = files.map((f) => {
...
| # | Title | Status |
|---|-------|--------|
${taList.map((t, i) => `| ${i + 1} | ${t.title} | ${t.status} |`).join('\n')}
...
// =============================================================================
async function handleTAInterviewResponse(state, response, workspaceFolder, taDir, stream) {
    switch (state.step) {
...
async function generateAndSaveTA(input, taDir, stream) {
    ...
    const content = TA_TEMPLATE
        ...
        .replace('{{IMPACTS}}', input.impacts.map(i => `| ${i.component} | ${i.impact} | ${i.description} |`).join('\n'))
        .replace('{{RISKS}}', input.risks.map(r => `| ${r.risk} | ${r.description} | ${r.mitigation} |`).join('\n'))
