...
// Topic suggestions for ADR content
...
`);
    ...
    if (request.command === 'quick') {
      ...
      const domain = await vscode.window.showQuickPick(
        ['Infrastructure', 'Data', 'Security', 'Integration', 'API', 'Other'],
      ...
      if (topic && TOPIC_SUGGESTIONS[topic]) {
        ...
        const selected = await vscode.window.showQuickPick(
          suggested.map(s => ({ label: s, picked: false })),
...
function detectTopic(title: string): string | null {
  ...
  if (lower.includes('kubernetes') || lower.includes('lambda') || lower.includes('container') || lower.includes('serverless') || lower.includes('infrastructure')) return 'infrastructure';
  if (lower.includes('auth') || lower.includes('security') || lower.includes('oauth') || lower.includes('jwt')) return 'security';
  return null;
}
...
function generateAdrContent(adr: AdrContent): string {
